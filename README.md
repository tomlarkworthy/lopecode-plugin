<!-- Generated by tools/channel/release-plugin.ts in tomlarkworthy/lopecode-dev.
...
Run the TypeScript directly under Bun, no build step:

```bash
bun run lopecode-channel.ts
bun run build.ts                            # produce dist/ for npm
node verify-node-build.mjs                  # MCP handshake + WebSocket pairing check
